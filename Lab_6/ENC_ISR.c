...
extern volatile uint8 ACCESS_TH;
extern volatile uint8 ACCESS_TL;
...
*
*			 	BELOW IS THE ALTERED CODE FOR THE ENCODER INTERRUPT 
*
********************************************************************************
*
* Summary:
...
CY_ISR(ENC_ISR_Interrupt)
{
    ...
    /* `#START ENC_ISR_Interrupt` */
        ...
        if (ENC_Read() == 1 && setTemp < 35) {   // checks if encoder is twisted clockwise and if set temp is below preset threshold (35)
            CyDelay(5);
            setTemp++;                           // increment set temp
        }    
    
        if (ENC_Read() == 3 && setTemp > 15) {   // checks if encoder is twisted counter-clockwise and if set temp is above preset threshold (15)
            CyDelay(5);
            setTemp--;                           // decrement set temp
        }
            
            tempArrayOne[0] = ACCESS_TL;                        // sets beginning of array to TL address
            tempArrayOne[1] = (uint8)(setTemp - (tolerance/2)); // sets 2nd byte of array to the low temp threshold (1 below the current temp)
            tempArrayOne[2] = 0;                                // sets the last byte to zero to let the I2C know its at the end of the array
    
            I2C_MasterWriteBuf(0x48, tempArrayOne, 3, I2C_MODE_COMPLETE_XFER);
            while (!(I2C_MasterStatus() & I2C_MSTAT_WR_CMPLT));
            I2C_MasterClearStatus();
        

            tempArrayOne[0] = ACCESS_TH;                        // sets beginning of array to TH address
            tempArrayOne[1] = (uint8)(setTemp + (tolerance/2)); // sets 2nd byte of array to high temp threshold (1 above the current temp)
            tempArrayOne[2] = 0;                                // sets the last byte to zero to let the I2C know its at the end of the array
            ...
            ENC_ClearInterrupt();               // clear interrupt
