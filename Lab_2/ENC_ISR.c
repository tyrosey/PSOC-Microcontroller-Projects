...
CY_ISR(ENC_ISR_Interrupt)
{
    ...
    if (ENC_Read() == 1) {                      // checks if encoder is twisted clockwise
    
		CyDelay(20);                            // delay 30 microseconds
		increment = 1;                          // sets increment flag to 1
    }    
    
    if (ENC_Read() == 3) {                      // checks if encoder is twisted counter-clockwise
        CyDelay(20);                            // delay 30 microseconds
        decrement = 2;                          // sets decrement flag to 2
    }
    
    ENC_ClearInterrupt();                       // clears the interrupt 
