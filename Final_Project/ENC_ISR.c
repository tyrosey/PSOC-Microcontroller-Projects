...
extern volatile int songCount;
extern volatile int levelCount;
...
CY_ISR(ENC_ISR_Interrupt)
{
    ...
    // shuffling through background songs
    if (screenCount == 0) {
        
        if (ENC_Read() == 1) {
            CyDelay(5);                 
        
            if (songCount == 4) {       // if at end of song list
                songCount = -1;         // set song list back to beginning
            }   
        songCount++;                    // increment songs if turned CW
        }
    
        if (ENC_Read() == 3) {
            CyDelay(5);
        
            if (songCount == 0) {       // if at beginning of song list
                songCount = 5;          // set song list back to the end
            }
        songCount--;                    // decrement songs if turned CCW
        }
    }
    
    //  shuffling through levels
    if (screenCount == 1) {
        if (ENC_Read() == 1) {
            CyDelay(5);
        
            // if level count is at end, dont increment any farther
            if (levelCount == 9) {
                levelCount = 8;
            }   
        ...
        if (ENC_Read() == 3) {
            ...
            // if level count is at beginning, dont decrement any farther
            if (levelCount == 1) {      
                levelCount = 2;         
            }
        levelCount--;                   // decrement level count
        }
    }
    ...
    if (screenCount == 3) {
        ...
        if (ENC_Read() == 1) {
            ...
            // if at end of letter couunt, dont increment any farther
            if (i == 25) {  
                i = 24;
            }   
        i++;                // increment letter count
        }
    
        if (ENC_Read() == 3) {
            CyDelay(5);
            
            // if letter coun tis at beginning, dont decrement any farther
            if (i == 0) {
                i = 1;
            }
        i--;                // decrement letter count
        }
    }
